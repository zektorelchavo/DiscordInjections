...
module.exports = class DiscordClient extends EventEmitter {
  constructor(DI) {
    ...
    this.channels.notFound = id => {
      const g = this.guilds.find(g => g.channels.get(id))
      if (g) {
  ...
  async onHandleWSMessage(ev) {
    ...
    switch (message.t) {
      case "READY":
        console.time("[DIIO] ready")
        console.debug("[DIIO] handling event", message.t, message.d)
        ...
        this._v = message.d.v
        */
        console.timeEnd("[DIIO] ready")
        break
      /*
      case "PRESENCE_UPDATE":
        this.users.set(message.d.user.id, message.d.user)
        break
*/
      default:
